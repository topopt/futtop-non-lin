...
% cgmax -
% cgmin -
function top3dmgcg_reference(nelx,nely,nelz,volfrac,penal,rmin,ft,nl,cgtol,cgmax,cgmin)
%
% example run command:
%
...
xPhys = linspace(0,1,nelx*nely*nelz)';
%utest = linspace(0,1,3*3*3*3)';
...
function [] = printState(u,nelx,nely,nelz)
fprintf('\nlet a :[][][][3]f64 = \n');
fprintf('[');
for i=1:(nelx+1)
    fprintf('[');
    for j=1:(nely+1)
        fprintf('[');
        for k=1:(nelz+1)
            n = (i-1)*(nely+1)*(nelz+1)+(k-1)*(nely+1)+(j-1);
            
            fprintf('[%25.25f,%25.25f,%25.25f]',u(3*n+1),u(3*n+2),u(3*n+3));
            if (k~=nelz+1)
                fprintf(',\n');
            end
        end
...
function [] = printStateFlat(u,nelx,nely,nelz)
fprintf('\nlet a :[][][][3]f64 = \n');
fprintf('[');
for i=1:(nelx+1)
    for j=1:(nely+1)
        for k=1:(nelz+1)
            ...
            fprintf('%25.25ff32,%25.25ff32,%25.25ff32',u(3*n+1),u(3*n+2),u(3*n+3));
            if (k~=nelz+1)
                fprintf(',\n');
            end
        end
...
function [] = printDens(x,nelx,nely,nelz)
fprintf('\nlet x :[][][]f32 = \n');
fprintf('[');
for i=1:(nelx)
    fprintf('[');
    for j=1:(nely)
        fprintf('[');
        for k=1:(nelz)
            n = (i-1)*(nely)*(nelz)+(k-1)*(nely)+(j-1);
            
            fprintf('%25.25ff32',x(n+1));
            if (k~=nelz)
                fprintf(',');
            end
        end
