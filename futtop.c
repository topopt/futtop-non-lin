...
void setupLoad(const int nelx, const int nely, const int nelz,
               const float forceMagnitude, float *F) {
...
void top3dmgcg(const uint_fast32_t nelx, const uint_fast32_t nely,
  ...
  setupLoad(nelx, nely, nelz, -0.002, F);
  ...
  while ((change > 1e-2) && (loop < iters)) {
    ...
    interTime = getTime();
    err =
        futhark_entry_solveSystemLinear(fut_ctx, &u_gpu, &cgres, &cgiter,
                                        xPhys_gpu, mgData, uold_gpu, force_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_context_sync(fut_ctx);
    handle_fut_err(fut_ctx, err);
    ...
    // compute new x
    interTime = getTime();
    err = futhark_entry_designIterationLinear(fut_ctx, &compliance, &change,
                                              &vol, &xnew_gpu, x_gpu, xPhys_gpu,
                                              u_gpu, volfrac, rmin);
    handle_fut_err(fut_ctx, err);
    err = futhark_context_sync(fut_ctx);
    handle_fut_err(fut_ctx, err);
  ...
  err = futhark_values_f32_4d(fut_ctx, uold_gpu, F);
  handle_fut_err(fut_ctx, err);
  err = futhark_context_sync(fut_ctx);
  handle_fut_err(fut_ctx, err);
  writeDensityAndDisplacement(nelx, nely, nelz, x, F, "linearResult.vtr");
