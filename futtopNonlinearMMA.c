...
void top3dmgcg(const uint_fast32_t nelx, const uint_fast32_t nely,
  ...
  struct futhark_f32_3d *xold_gpu =
      futhark_new_f32_3d(fut_ctx, x, nelx, nely, nelz);
  struct futhark_f32_3d *xold2_gpu =
      futhark_new_f32_3d(fut_ctx, x, nelx, nely, nelz);
  struct futhark_f32_3d *lowold_gpu =
      futhark_new_f32_3d(fut_ctx, x, nelx, nely, nelz);
  struct futhark_f32_3d *uppold_gpu =
      futhark_new_f32_3d(fut_ctx, x, nelx, nely, nelz);
  struct futhark_f32_3d *xPhys_gpu;
  struct futhark_f32_3d *xnew_gpu;
  struct futhark_f32_3d *xonew_gpu;
  struct futhark_f32_3d *xo2new_gpu;
  struct futhark_f32_3d *low_gpu;
  struct futhark_f32_3d *upp_gpu;
  ...
  while ((change > 1e-2) && (loop < iters)) {
    ...
    // compute new x
    interTime = getTime();
    err = futhark_entry_designIterationMMA(
        fut_ctx, &compliance, &change, &vol, &xnew_gpu, &xonew_gpu, &xo2new_gpu,
        &low_gpu, &upp_gpu, &lambda_gpu, loop, x_gpu, xold_gpu, xold2_gpu,
        xPhys_gpu, u_gpu, force_gpu, lambdaold_gpu, lowold_gpu, uppold_gpu,
        volfrac, rmin);
    ...
    handle_fut_err(fut_ctx, err);
    err = futhark_free_f32_3d(fut_ctx, xold_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_free_f32_3d(fut_ctx, xold2_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_free_f64_4d(fut_ctx, lambdaold_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_free_f32_3d(fut_ctx, lowold_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_free_f32_3d(fut_ctx, uppold_gpu);
    handle_fut_err(fut_ctx, err);
    err = futhark_context_sync(fut_ctx);
    handle_fut_err(fut_ctx, err);
